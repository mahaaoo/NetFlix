    ...
    private func configureHeroHeaderView() {
        APICaller.shared.getTrendingMovies { [weak self] result in
            ...
            case .success(let titles):
                let seletedTitle = titles.randomElement()
                self?.randomTrendingMovie = seletedTitle
                self?.headerView?.configure(with: TitleViewModel(titleName: seletedTitle?.original_title ?? "", posterURL: seletedTitle?.poster_path ?? ""))
